...
################## self defined periodic activation function ##################
class PeriodicActivationFunction(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input):
        ctx.save_for_backward(input)
        output = torch.fmod(input + torch.pi, 2 * torch.pi) - torch.pi
    ...
    def backward(ctx, grad_output):
        ...
        discontinuous_points = torch.fmod(input + torch.pi, 2 * torch.pi) - torch.pi
        grad_input[discontinuous_points.abs() < 1e-6] = 1 / torch.pi
        return grad_input,


def periodic_activation(x):
    return PeriodicActivationFunction.apply(x)
...
class PPO:
    ...
    def update(self):
        ...
        # personal change: from normalizing rewards to normalizing advantages
